...
import { OficinaViews } from "./oficinaView";
...
export function Oficina() {
  ...
  const tokenData = accessToken ? jwt_decode(accessToken) : null; // Verifica si hay un token antes de decodificarlo
...
localStorage.removeItem("Rol");
  ...
  // Verifica si el array "roles" contiene el rol "RRHH"
  const hasRole = roles && roles.includes("RRHH");

  return (
    <>
      {isAuthenticated && hasRole ? (<OficinaViews />) : (<Navigate to="/Login" />)}
    </>
  );
}
