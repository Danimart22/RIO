...
import { PorteriaViews } from "./porteriaView";
...
tokenData && new Date(tokenData.expiry) > new Date()
  ...
  let roles = userRoles();
  // Verifica si el array "roles" contiene el rol "RRHH" o "porteria"
  const hasRole = roles?.includes("porteria") || roles?.includes("RRHH");

  return (
    <>
      {isAuthenticated && hasRole ? (<PorteriaViews />) : (<Navigate to="/Login" />)}
    </>
  );
}