...
import "./PorteriaViews.css";

export function PorteriaViews() {
  ...
  const VisitanteIcon = ({ size = 20 }) => (
    <Icon path={mdiBriefcaseAccountOutline} size={size / 19} color="currentColor" />
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 180,
      renderCell: (params) => (
        <div className="porteria-cell-scroll">
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (tipo === "usuario") return <div title="Usuario"><UsuarioIcon /></div>;
        if (tipo === "visitante") return <div title="Visitante"><VisitanteIcon /></div>;
        return tipo;
      },
    },
  ];
        ...
        oficina.reservas.map((reserva) => ({
          ...
          "Hora de Llegada":
            reserva.horaLlegada.length > 0
              ...
              : reserva.horaSalida.length > 0 ? "" : "Reservó pero NO asistió",
          "Hora de Salida":
            reserva.horaSalida.length > 0
              ? reserva.horaSalida.join(", ")
              : reserva.horaLlegada.length > 0 ? "" : "Reservó pero NO asistió",
        }))
      );
  ...
  return (
    <Fragment>
      <div className="container">
        <div
          className="porteria-skeleton-container"
          style={{ display: loading ? "block" : "none" }}
        >
          <Skeleton variant="text" height={80} width={"35%"} />
          <div className="porteria-skeleton-buttons">
            <Skeleton variant="text" height={50} width={"27%"} className="porteria-skeleton-btn" />
            <Skeleton variant="text" height={50} width={"15%"} className="porteria-skeleton-btn" />
          </div>
        ...
        <div style={{ display: loading ? "none" : "block" }}>
          <div>
            <h1>
              Asientos Disponibles: {asientosDisponibles} (Asientos de Gerentes: {puestosGerentes})
            </h1>
          </div>
          <div className="porteria-table-wrapper table-responsive">
            <div className="porteria-toolbar">
              <DatePicker
                selected={selectedDate}
                onChange={(date) => setSelectedDate(date)}
                dateFormat="YYYY/MM/DD"
                placeholder="Selecciona una fecha"
                className="porteria-datepicker"
              />
              <Button
                className="porteria-export-button"
